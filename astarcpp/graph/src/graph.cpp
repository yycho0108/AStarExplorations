...
namespace graph {
    ...
    template <typename VertexProperty = boost::no_property, typename EdgeProperty = boost::property<boost::edge_weight_t, float>>
    using GraphBase = boost::adjacency_list<
        boost::vecS, boost::vecS, boost::undirectedS, VertexProperty,
        EdgeProperty>;
    ...
    public:
        astar_goal_visitor(const Vertex& goal)
            : m_goal(goal)
        {
        }
    ...
    auto BuildGridGraph(const MatrixXu& grid)
    {
        ...
        std::unordered_map<Eigen::Vector2i, Vertex, matrix_hash<Eigen::Vector2i>> ivmap;
    ...
    auto BuildGraph(
        const MatrixXu& grid,
        cho::graph::Graph2D* const g_ptr,
        std::unordered_map<Eigen::Vector2i, std::remove_reference<decltype(*g_ptr)>::type::vertex_descriptor, matrix_hash<Eigen::Vector2i>>* const ivmap)
    {
        // Type alias
        ...
        std::unordered_map<Eigen::Vector2i, Vertex, matrix_hash<Eigen::Vector2i>> vmap;
            ...
            for (int j = 0; j < grid.cols(); ++j) {
                ...
                for (int di = -1; di <= 1; ++di) {
                    for (int dj = -1; dj <= 1; ++dj) {
                        ...
                        const Edge& e = boost::add_edge(vmap.at({ i, j }), vmap.at({ i2, j2 }), g).first;
    ...
    VecVector2i FindPath(const MatrixXu& grid, const Eigen::Vector2i& iv0, const Eigen::Vector2i& iv1)
    {
        ...
        boost::unordered_map<Vertex, std::pair<float, float>, boost::hash<Vertex>> loc_map;
        ...
        try {
            boost::astar_search_tree(g1, v0,
                distance_heuristic<Graph, Vertex, std::remove_reference<decltype(GetXy)>::type, float>(GetXy, v1),
